...
public interface SubSampleRepository extends CrudRepository<SubSample, Long>{
	@Transactional
  	@Modifying
	@Query("SELECT id, name, description, danger, sample FROM SubSample s where s.name=? ")
...
//@Query("SELECT id, name, description, project FROM Sample s where s.getProject().getId()=?")
	@Query("SELECT id, name, description, danger, sample  FROM SubSample s where s.sample.id=?")
